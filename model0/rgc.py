...
class RetinalGanglionCell:
    def __init__(
        self, cell_type: str, receptive_field_size: float, response_characteristic: str
    ) -> None:
...
class MidgetRGC(RetinalGanglionCell):
    def __init__(self, receptive_field_size: float) -> None:
        super().__init__(
    ...
    def temporal_response(self, time_series: List[float]) -> float:
        if not time_series:
            return 0.0
        kernel: np.ndarray = np.ones(10) / 10  # Simple moving average
        response: np.ndarray = np.convolve(time_series, kernel, mode="valid")
...
class SmallBistratifiedRGC(RetinalGanglionCell):
    ...
    def respond_to_light(self, light_intensity, surround_intensity, time_series):
        center_weight = 0.7
        surround_weight = -0.3
        ...
        return (
            f"Small Bistratified RGC net response: {net_response:.2f} spikes/sec, "
            f"Temporal response: {self.temporal_response(time_series):.2f} spikes/sec"
        )

    def temporal_response(self, time_series):
        if not time_series:
            return 0.0
        kernel = np.cos(np.linspace(0, np.pi, 10)) * 0.5
