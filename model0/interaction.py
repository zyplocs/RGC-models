...
def create_cell() -> Optional[RetinalGanglionCell]:
    print(
        """
    -- Welcome to RGC model0 --
    """
    )
    print(
        "You will be prompted to create a Retinal Ganglion Cell (RGC) by selecting its type and receptive field size."
    )
    print("Types of RGCs:")
    ...
    * You will see the net response of the cell in spikes per second,\n
    ...
    while True:
        ...
        surround_intensity: str = input(
            "Enter surround light intensity (in cd/m²): "
        ).strip()
        ...
        try:
            ...
            if len(time_series) > 10:  # Keep the time series to a fixed length
                time_series.pop(0)
            response = cell.respond_to_light(
                light_intensity_value, surround_intensity_value, time_series
            )
            print(response)
            print("\nExplanation:")
            print(
                "Net Response: This value indicates the overall firing rate of the cell in response to the given light intensities."
            )
            print(
                "Temporal Response: This value shows how the firing rate has changed over the recent history of light intensities."
            )
        except ValueError:
